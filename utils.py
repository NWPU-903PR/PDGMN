...
def create_G_from_dataframe(net,is_max_component=False):
    G = nx.Graph()
    s_ls = [e[0] for e in net]
    t_ls = [e[1] for e in net]
    edges = [(s_ls[i],t_ls[i]) for i in range(0,len(s_ls))]
    G.add_edges_from(edges)
    if is_max_component:
        components_gg = nx.connected_components(G)
        max_component = max(components_gg, key=len)
        subG = G.subgraph(max_component)
        return subG
    else:
        return G
